...
### Nonce
First of all we want to get the user's nonce from the `CounterForwarder` contract that we deployed [earlier](/docs/ethereum/meta-transactions/example/forwarder).
...
In our case we want a dedicated server to execute the Meta Transaction on behalf a user.
