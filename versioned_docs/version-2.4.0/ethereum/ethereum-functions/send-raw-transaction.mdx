...
### `Sign Transaction`
Signs a transaction that can be submitted to the network at a later time using `eth_sendRawTransaction` as long as the nonce and the
...
Signs a type 2 transaction that can be submitted to the network at a later time using `eth_sendRawTransaction` as long as the nonce and the
...
* Max Priority Fee Per Gas <Step text="4"/> : The maximum amount the user is willing to pay as a tip to miners (in Wei). Incentivizes prioritization of the transaction.
* Max Fee Per Gas <Step text="5"/> : The maximum total amount the user is willing to pay for the transaction in total (in Wei). Includes base fee and priority fee.
